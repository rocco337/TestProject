...
namespace SimpleValidator
{
    public class ObjectValidatorBase<T>
    {
        ...
        public bool IsValid()
        {
            ...
            var invalidResults = _rules.Where(m => m.OnlyIf == null || (m.OnlyIf != null && m.OnlyIf(_model)));

            //select all rules that arent satisfied
            invalidResults = invalidResults.Where(m => !m.ValidationExpression(_model));
        ...
        protected ObjectValidatorBase(T model)
        {
            _model = model;   
        }
